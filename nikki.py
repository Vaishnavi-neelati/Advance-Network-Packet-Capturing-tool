...
# Define mapping for protocol filters
protocol_filters = {
...
def packet_callback(packet):
    if IP in packet:
        for proto_name, proto_filter in app.get_selected_protocol_filters().items():
...
class PacketSnifferApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Advanced Network Packet Sniffer with Protocol Filters")
        ...
        self.protocol_vars = {}
        row = 0
        for i, proto in enumerate(protocol_filters.keys()):
            var = tk.BooleanVar(value=True)
            chk = ttk.Checkbutton(self.frame, text=proto, variable=var)
            chk.grid(row=row, column=0 if i < 4 else 1, sticky="w", padx=5)
        ...
        self.tree.grid(row=row+1, column=0, columnspan=3, pady=10)
    ...
    def get_selected_protocol_filters(self):
